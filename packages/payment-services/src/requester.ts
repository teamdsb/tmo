...
import type { ApiClientRequestOptions, ApiClientRequester, ApiClientResponse } from '@tmo/payment-api-client'
...
const toApiError = (statusCode: number, data: unknown, headers?: Record<string, string>, raw?: unknown): ApiError => {
  const payload = (data && typeof data === 'object') ? (data as { code?: string; message?: string; requestId?: string; details?: Record<string, unknown> }) : undefined
  return new ApiError(payload?.message ?? 'request failed', statusCode, {
    code: payload?.code,
    requestId: payload?.requestId,
    details: payload?.details,
    raw: raw ?? data ?? headers
  })
}
