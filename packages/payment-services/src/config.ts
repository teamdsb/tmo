import type { ApiClientRequester } from '@tmo/payment-api-client'
...
export const resolveBaseUrl = (value?: string): string => {
  ...
  if (typeof process !== 'undefined' && process?.env) {
    return firstNonEmpty(process.env.TARO_APP_API_BASE_URL, process.env.TARO_APP_PAYMENT_BASE_URL)
  }
  return ''
}
...
export const resolveDevToken = (value?: string): string | undefined => {
  ...
  if (typeof process !== 'undefined' && process?.env) {
    return process.env.TARO_APP_PAYMENT_DEV_TOKEN ?? process.env.TARO_APP_COMMERCE_DEV_TOKEN
  }
  return undefined
}
