...
let paymentApiClientConfig: ApiClientConfig | null = null

export const setPaymentApiClientConfig = (config: ApiClientConfig): void => {
  paymentApiClientConfig = config
}

export const getPaymentApiClientConfig = (): ApiClientConfig => {
  if (!paymentApiClientConfig) {
    throw new Error('Payment api client is not configured')
  }
  return paymentApiClientConfig
}
...
export const apiMutator = async <T>(url: string, options: RequestInit): Promise<T> => {
  const { baseUrl, requester } = getPaymentApiClientConfig()
