...
## 扩展阅读
- [客户账期模块记录（业务+技术双视角）](./账期模块-客户财务档案记录.md)
...
- 客户运营标记：管理员后台可给客户打多个标签（如重点客户、待回访等），支持筛选与批量操作。

### 模块四：售后模块 + AI 预留
- 售后入口：小程序内有售后服务入口，客户提交售后问题/反馈。
- 售后记录管理：后台查看、标记、跟进售后记录。
- 预留 AI 接口：系统预留接口，后续接大语言模型，实现 AI + 人工客服一起回复。
- 在线客服：从“我的”页进入独立客服页，采用电商 IM 样式；支持文本、图片、订单卡片、商品卡片。后台坐席必须显式认领会话后才能已读或回复；商品卡片从已上架目录中搜索选择，客户点击后进入对应小程序商品详情。
- 角色边界：客户在小程序内联系 `CS` 客服；`SALES` 只保留在小程序侧做客户跟进，不承担后台客服沟通。
...
### 3.1 客户下单主流程（小程序）
- 分类/搜索 → 商品详情 → 加入购物车
- 填写收货地址 → 选择线上或线下付款 → 提交意向订单；付款方式提交后按订单锁定
...
- 线上付款：生成支付订单 → 微信支付 → 回调更新状态 → 前端展示支付状态/记录
- 线下付款：订单创建后保持未支付，等待管理后台确认到账；不能为该订单创建线上支付会话
...
### 3.7 Admin 线下收款与订单派发
- BOSS、MANAGER、ADMIN 可为待付款订单选择 active SALES 并填写必填备注，一次确认全额线下收款并派单。
- 只有客户选择了线下付款的未支付订单可确认线下收款；线上未支付订单需等待渠道支付完成。
- 订单列表的支付列同时展示付款方式和到账状态，交易链接只使用真实线上支付 ID。
- 已在线支付订单可直接派单；订单发货前可改派，发货、送达、取消或关闭后不可修改负责人。
- 每次成功操作写入不可随订单页面刷新丢失的审计事件；相同幂等键重试不会重复写事件。
- CS、SALES、CUSTOMER 无权执行该操作。手工确认线下付款不可在本功能内撤销；退款或冲正需要独立流程。

### 3.8 新增：下单人批量导入加购（确定的自动，不确定的引导确认）
- 下单人上传 Excel（50 个不同物品）→ 系统尝试匹配商品/规格
- 唯一确定的行直接加购
- 不确定的行进入确认界面（可优化呈现方式，目标是降低申购人工作量）

### 3.9 小程序登录与业务员工作台
- 登录后不展示“以客户身份进入 / 以业务员身份进入”中间选择页。
- 同时拥有 `CUSTOMER` 与 `SALES` 的账号默认以 `CUSTOMER` 进入商城；仅有 `SALES` 的账号直接以 `SALES` 进入。
- 具有 `SALES` 身份的用户仍从“我的”页进入业务员工作台；工作台顶部必须避让小程序胶囊，底部导航必须通过共享安全区组件适配设备底部系统 UI。
- 商城首页、分类、购物车和“我的”使用平台原生 TabBar。原生 TabBar 独占主标签页的底部系统安全区；独立页面的固定操作栏和底部弹层通过共享组件消费一次设备底部安全区，业务页面不得猜测原生 TabBar 高度或自行叠加安全区。

### 3.10 商品三级规格与目录往返

- 商品导入支持标准中英文模板、试运行母表和历史五列业务表，使用上传、识别预览、确认、结果和持续复核流程。明确商品可包含很多实际 SKU 组合，最多三级，不强制拆分完整型号。
- 无法识别的新行独立创建单 SKU 草稿并持续提醒管理员；未知分类进入待分类，未知尺寸单位不得推测。重复文件不重复建商品，不覆盖人工修正；空白可选字段默认保留。
- Admin 工作台集中商品导入、导出和分页任务历史；商品列表提供待复核筛选与编辑处理入口。商品复核完成前不能上架，批量导出可只选择待复核商品。
- 每个商品在 admin 独立设置 1～3 级名称，例如“材质 → 长度 → 直径”，逐行维护实际 SKU 组合、编码、启用状态和价格。
- 支持新增层级、修改名称、删除末级；超过三级、空白/重复名称、缺少规格值或有效组合冲突不得保存。名称变化不改变 SKU 身份或未修改的价格。
- 小程序详情与购物车换规格按名称逐级选择，只展示有效组合，切换上级清空后续选择；规格摘要展示完整路径。
- 商品目录可以按当前筛选导出所有匹配页，与商品导入使用同一 Excel 模板；显式列为一级至三级名称和值，包含稳定 ID、图片 URL、状态和阶梯价。重复回导更新原记录，不生成重复商品或 SKU。
- 无 SKU 商品、无编码 SKU 和停用 SKU 可完整导出回导。保留原有采购需求导出流程；导入逐商品组事务处理，失败组不留下部分商品数据。
