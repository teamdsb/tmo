...
- 商品导入先持久预检，明确确认后再按商品事务写入。标准中英文模板、试运行母表、旧五列业务表归一到同一规格校验；无法确定归属或规格的新行独立为 DRAFT，完整型号允许一级。维度单位未知时不猜测。
- 可选字段空白默认保留；清空必须在预览明确选择。来源绑定与商品/SKU 同事务保存，来源键不包含文件名或行号。已有来源内容未变化时跳过，避免重导覆盖人工修正。
- 待复核记录与商品状态分开持久保存，记录来源与具体原因。普通保存不自动完成复核；存在待复核问题时商品编辑和回导均不能将商品设为 ACTIVE。没有价格不新增上架限制，沿用询价业务。
