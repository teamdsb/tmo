...
- [x] (2026-01-23 16:10Z) 读取并对齐现有材料：`docs/context/product-requirements.md`、`docs/context/rbac.md`、`contracts/openapi/identity.yaml`、仓库各 README；并记录关键技术抉择（见 `Decision Log`）。
...
- Observation: 如果未来在 commerce 中开启鉴权并签发 `SALES` token，当前 commerce 的订单读接口没有正确实现 `docs/context/rbac.md` 的 OWNED 约束（存在越权风险）。同时，commerce 下单时目前把 `orders.owner_sales_user_id` 写为 NULL，导致“客户归属到业务员后，下单算在业务员下面”的业务目标无法成立。
...
5) RBAC（Role-Based Access Control）：基于角色的访问控制。`docs/context/rbac.md` 定义了系统角色（CUSTOMER/SALES/PROCUREMENT/CS/ADMIN）以及“scope”概念（SELF/OWNED/ALL）。identity v0 先把“角色出现在 JWT 与 /me 响应里”跑通；权限码与 scope 评估属于长线里程碑。
...
6) `audit_logs`（v0 先落表与写入最小事件）：记录 login、bind 等事件，满足 `docs/context/rbac.md` 的审计方向（长线会扩展到跨服务动作）。
...
2) 在 `services/commerce/internal/http/handler/orders.go` 下单时写入 owner 快照：
   ...
   - 这一步是为了满足 `docs/context/product-requirements.md` 的“客户扫码归属业务员后，后续下单计入该业务员下面”，以及 `docs/context/rbac.md` 的 OWNED scope 查询基础。
...
以下命令假设从仓库根目录执行。每一步都应可重复执行；如果某步失败，先按本计划的 “Idempotence and Recovery” 处理，再继续。
