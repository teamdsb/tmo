...
- [ ] (下一里程碑) 完成 P1-供应商域（契约、网关、服务实现、前端接线、回归）。
...
本轮已经把 P0 三个页面从“静态/半 mock”推进到“可真实请求的闭环形态”，且已通过后端测试与前端构建验证。当前主要风险从“接口缺失”转为“域模型深度不足”：尤其是支付运营数据持久化、供应商/报价/客服/设置四个 P1 域尚未建立。

下一阶段不应继续扩散页面改动，而应以 P1 单域里程碑推进，每个里程碑坚持“契约冻结 -> 网关分流 -> 服务实现 -> 前端接线 -> 行为验收 -> 回归测试”。
