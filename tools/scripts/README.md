...
- `test-backend-coverage.sh`: run `go test -coverprofile` for every Go module under `services/` and `packages/`, then emit per-module and combined coverage summaries into `.logs/backend-coverage/` (`BACKEND_COVERAGE_DIR` to override).
