...
test('hybrid fulfillment refreshes payment, owner and audit timeline', async ({ page }) => {
  let paid = false;
  await page.route(/\/(?:api\/)?orders(?:\?|$)/, (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ items: [{ id: orderId, status: paid ? 'CONFIRMED' : 'SUBMITTED', paymentStatus: paid ? 'PAID' : 'UNPAID', paymentMethod: 'OFFLINE', paymentChannel: paid ? 'OFFLINE' : null, ownerSalesUserId: paid ? salesId : null, address: { receiverName: '客户', receiverPhone: '1', detail: '地址' }, items: [], createdAt: '2026-07-06T10:00:00Z' }], page: 1, pageSize: 20, total: 1 }) }));
  ...
  await page.route(`**/api/admin/orders/${orderId}/fulfillment`, async (route) => { paid = true; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ id: orderId, status: 'CONFIRMED', paymentStatus: 'PAID', paymentMethod: 'OFFLINE', paymentChannel: 'OFFLINE', ownerSalesUserId: salesId, address: { receiverName: '客户', receiverPhone: '1', detail: '地址' }, items: [], createdAt: '2026-07-06T10:00:00Z' }) }); });
