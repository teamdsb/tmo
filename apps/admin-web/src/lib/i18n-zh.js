const RAW_REPLACEMENTS = [
  ...
  ['All values are from backend endpoints.', '所有数据均来自后端接口。'],
  ...
  ['Role-based Access', '基于角色的访问'],
  ['No dedicated RBAC analytics endpoint is wired for dashboard cards.', '仪表盘卡片尚未接入专用 RBAC 分析接口。'],
  ['Pending Import Tasks', '待处理导入任务'],
  ['Use the Logistics page to query real import jobs by job ID.', '请在物流页面通过任务 ID 查询真实导入任务。'],
  ['Orders (Live)', '订单（实时）'],
  ['Backend order data only.', '仅展示后端订单数据。'],
  ['Order List', '订单列表'],
  ['Order ID', '订单号'],
  ...
  ['No dedicated live logistics detail panel is wired for this page.', '该页面尚未接入专用实时物流详情面板。'],
  ['Sourcing Inquiries (Live)', '需求订单询价（实时）'],
  ['Backend inquiry records only.', '仅展示后端询价记录。'],
  ['Inquiry List', '询价列表'],
  ['Inquiry ID', '询价编号'],
  ['Message', '消息'],
  ['Created At', '创建时间'],
  ...
  ['No dedicated live detail thread endpoint is wired for this page.', '该页面尚未接入专用实时详情流接口。'],
  ['Import Jobs (Live)', '导入任务（实时）'],
  ['Backend import/export and feature-flag operations only.', '仅保留后端导入导出与功能开关操作。'],
  ...
  ['Static parsing progress and fake candidate rows are hidden. Use job APIs above for real progress.', '静态解析进度和模拟候选行已隐藏，请使用上方任务接口查看真实进度。'],
  ...
  ['Administrator login roles are supported.', '当前支持老板、经理、管理员和客服登录。'],
  ['Failed to load', '加载失败'],
  ['updated:', '更新时间：'],
  ['Warnings', '告警'],
  ['Support', '支持'],
