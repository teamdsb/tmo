...
const resolveStaffDisplayName = (staffOptions, userId, fallback = '-') => {
  if (!userId) return fallback;
  const matched = staffOptions.find((item) => item.id === userId);
  if (matched?.displayName) {
    return matched.displayName;
  }
  return String(userId).slice(0, 8) || fallback;
};
...
export const SupportWorkspacePage = () => {
  ...
  }, [activeConversation?.ownerSalesUserId, staffOptions]);

  const assigneeLabel = useMemo(() => {
    return resolveStaffDisplayName(staffOptions, activeConversation?.assigneeUserId, activeConversation?.assigneeRole || '待领取');
  }, [activeConversation?.assigneeRole, activeConversation?.assigneeUserId, staffOptions]);
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 truncate text-xs text-slate-500">
                    {customerPhoneLabel(conversation)} · 归属销售 {resolveStaffDisplayName(staffOptions, conversation.ownerSalesUserId, '未分配')}
                  </p>
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    {conversation.staffUnreadCount > 0 ? (
                      <span className="rounded-full bg-rose-500 px-2 py-0.5 text-[10px] font-semibold text-white" data-testid={`support-conversation-unread-${conversation.id}`}>
        ...
        <aside className="col-span-3 flex min-h-0 flex-col gap-4">
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm" data-testid="support-customer-card">
            ...
            <dl className="mt-4 space-y-3 text-sm text-slate-600">
              ...
              <div>
                <dt className="text-xs uppercase tracking-wide text-slate-400">手机号</dt>
                <dd className="mt-1 font-medium text-slate-900">{activeConversation ? customerPhoneLabel(activeConversation) : '--'}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wide text-slate-400">客户 ID</dt>
                <dd className="mt-1 font-medium text-slate-900">{activeConversation?.customerUserId || '--'}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wide text-slate-400">归属销售</dt>
                <dd className="mt-1 font-medium text-slate-900">{ownerSalesLabel}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wide text-slate-400">当前坐席</dt>
                <dd className="mt-1 font-medium text-slate-900">{assigneeLabel}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wide text-slate-400">会话状态</dt>
                <dd className="mt-1 font-medium text-slate-900">{activeConversation ? getSupportConversationStatusLabel(activeConversation.status) : '--'}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wide text-slate-400">最近消息</dt>
                <dd className="mt-1 font-medium text-slate-900">{activeConversation?.lastMessageAt ? formatSupportTime(activeConversation.lastMessageAt, true) : '--'}</dd>
              </div>
            </dl>
          </section>
